...
const Categorie = ({categorie, title}) => {
  ...
  const images = useFetchData(`${process.env.REACT_APP_URL}api/images/read_category.php?category=${categorie}`)

  return !images[1]
    ?<motion.div variants={fadeAnim} animate="visible" initial="hidden" exit="exit" className="categorie-container">
      <Helmet>
        <title>{title}</title>
        <meta name="description" content={`Page présentant mes photo de la catégorie suivante: ${title}`}/> 
      </Helmet>
      <h1>{title}</h1>
