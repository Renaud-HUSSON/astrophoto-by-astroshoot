...
const ImageDetails = () => {
  ...
  const imageData = useFetchData(
    `${process.env.REACT_APP_URL}/api/images/read_single.php?id=${params.id}`
  );
  ...
  return !imageData[1] ? (
    ...
    >
      <Helmet>
        ...
        <meta
          name="description"
          content={`Détails de ma photo suivante: ${image.titre}`}
        />
      </Helmet>
      <h1>{image.titre}</h1>
        ...
        <div className="image">
          {image.src.match(/[.]mp4/) ? (
            <video
              controls
              autoPlay
              src={`https://astrophoto-amateur.com/${image.src}`}
              type="video/mp4"
            ></video>
          ) : (
            <img
